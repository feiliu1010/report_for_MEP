...
;Modified by Fei Liu (Aug 3,2013)
;Please do not distribute the code to others without author's consent.

;the filter is the same as the 0.01 deg work
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;print,'list_omi',list_omi

dir_out='/home/liufei/Data/Satellite/NO2/L2S_GEO/'+ Yr4+'/'

;Filtering parameters
large_pixels=1 ; 1: remove large pixels; 0: take into account each pixel
if large_pixels EQ 1 then lp=10 else lp=0 ; lp represents the number of edge pixels you want to remove on each side
missing=-999.
...
get_dpgc_data_NASA, list_omi(k), mtime, spx, sza, vzen, azi, vazi, $
  latcorner, loncorner, g_lat, g_lon,  $
  ;trp_flg, $
  no2trp, no2trpstd, cfr, cldpre,  sht, crd, salb, errcol
...
;    if prev_MM LT 10 then month='0'+strtrim(prev_MM,2) else month=strtrim(prev_MM,2)
    if prev_MM LT 10 then Mon2='0'+strtrim(prev_MM,2) else Mon2=strtrim(prev_MM,2)
    if prev_DD LT 10 then day='0'+strtrim(prev_DD,2) else day=strtrim(prev_DD,2)
    ; Write netCDF file
    ;-----------------------------------------------------------------------
        Out_nc_file=dir_out+'OMI_' + strmid(strtrim(dx,2),0,4) + 'x' + $
      strmid(strtrim(dy,2),0,4)+'_'+strtrim(prev_YY,2)+'_'+Mon2+'_'+day+'_sza70_crd50_v2.nc'
        FileId = NCDF_Create( Out_nc_file, /Clobber )
        NCDF_Control, FileID, /NoFill
        xID   = NCDF_DimDef( FileID, 'X', GEOS_Grid.IMX )
        yID   = NCDF_DimDef( FileID, 'Y', GEOS_Grid.JMX  )
        ...
        NCDF_Attput, FileID, /Global, 'Title', 'daily mean'
        NCDF_Control, FileID, /EnDef
        NCDF_VarPut, FileID, LonID, grid_lon_mid ,   Count=[ GEOS_Grid.IMX ]
        NCDF_VarPut, FileID, LatID, grid_lat_mid ,   Count=[ GEOS_Grid.JMX ]
        NCDF_VarPut, FileID, VCDID, omi_array,      Count=[ GEOS_Grid.IMX, GEOS_Grid.JMX ]
        NCDF_VarPut, FileID, SamID, sample_array,    Count=[ GEOS_Grid.IMX, GEOS_Grid.JMX ]
        NCDF_Close, FileID
...
; Anomaly 5:(Since July, 5th 2011) ==> 42:45
; Anomaly 6:(Since Aug, 2011) ==> 41:45
IF (anomalies EQ 1) THEN BEGIN
    ...
    ENDIF ELSE IF ((yy EQ 2011) AND (MM GE 7)) THEN BEGIN
        no2trp(42:45,*) = missing
    ENDIF ELSE IF ((yy EQ 2011) AND (MM GE 8)) THEN BEGIN
        no2trp(41:45,*) = missing
    ENDIF ELSE IF ((yy GE 2012)) THEN BEGIN
        no2trp(41:45,*) = missing
    ENDIF 

ENDIF

print,'removing boundary orbit'
IF yy LE 2008  THEN BEGIN
        no2trp(0:9,*) = missing
        no2trp(50:59,*) = missing
ENDIF ELSE IF ((yy GE 2009) AND (yy LE 2010)) THEN BEGIN
        no2trp(0:9,*) = missing
        no2trp(26:59,*) = missing
ENDIF ELSE IF yy GE 2011  THEN BEGIN
        no2trp(0:9,*) = missing
        no2trp(25:59,*) = missing
ENDIF

npix = N_ELements(no2trp[*,0])
 
for m=0,n_elements(g_lon(0,*))-1 do begin
    lon_NO2_1(n:n+(npix-lp)-1)=loncorner(2,lp/2:npix-(lp/2+1),m)
    lat_NO2_1(n:n+(npix-lp)-1)=latcorner(2,lp/2:npix-(lp/2+1),m)
    lon_NO2_2(n:n+(npix-lp)-1)=loncorner(1,lp/2:npix-(lp/2+1),m)
    lat_NO2_2(n:n+(npix-lp)-1)=latcorner(1,lp/2:npix-(lp/2+1),m)
    lon_NO2_3(n:n+(npix-lp)-1)=loncorner(3,lp/2:npix-(lp/2+1),m)
    lat_NO2_3(n:n+(npix-lp)-1)=latcorner(3,lp/2:npix-(lp/2+1),m)
    lon_NO2_4(n:n+(npix-lp)-1)=loncorner(0,lp/2:npix-(lp/2+1),m)
    lat_NO2_4(n:n+(npix-lp)-1)=latcorner(0,lp/2:npix-(lp/2+1),m)
...
;;    flg_NO2(n:n+(npix-lp)-1) = trp_flg(lp/2:npix-(lp/2+1),m)
    ...
    cfr_NO2(n:n+(npix-lp)-1) = cfr(lp/2:npix-(lp/2+1),m)
    salb_NO2(n:n+(npix-lp)-1) = salb(lp/2:npix-(lp/2+1),m)
...
;ind_ok = where((time_NO2 GT 0) AND (flg_NO2 EQ 0) AND (FINITE(value_NO2) EQ 1) AND (sza_NO2 LT 70.) AND $
 ind_ok = where((time_NO2 GT 0) AND  (value_NO2 ge 0) AND (sza_NO2 LT 70.) AND (sza_NO2 GE 0.)$
              ;(crd_NO2/100. GE 0. AND crd_NO2/100. LE 50.) AND             $
	       (cfr_NO2/1000. GE 0. AND cfr_NO2/1000. LE 0.3) AND             $
               (salb_NO2/1000. GE 0. AND salb_NO2/1000. LE 0.3) AND             $
              ; (tramf_NO2 GT 0.5) 
...
print,'number of ind_ok',n_elements(ind_ok)
...
for nn=0L,n_elements(lon_NO2_1_unit)-1 do begin
 ...
 for pp=0L,n_elements(polygons_ind)-2L do begin
     ...
     for ii=0L,n_elements(ind_grid_x)-1L do begin
         ...
         sample_array(ind_grid_x(ii),ind_grid_y(ii))+=1
	endfor
 endfor
