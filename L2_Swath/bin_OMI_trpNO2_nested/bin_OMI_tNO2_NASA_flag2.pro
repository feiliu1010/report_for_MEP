...
@polyfillaa
@get_dpgc_data_NASA_flag2

PRO bin_OMI_tNO2_NASA_flag2
...
dir_out='/home/liufei/Data/Satellite/NO2/L2S_GEO/flag2/'+ Yr4+'/'
...
xtrack_flag_NO2  = FltArr( ARRSIZE         )
...
get_dpgc_data_NASA_flag2, list_omi(k), mtime, spx, sza, vzen, azi, vazi, $
  latcorner, loncorner, g_lat, g_lon,  $
  ;trp_flg, $
  no2trp, no2trpstd, cfr, cldpre,  sht, crd, salb, errcol,xtrack_flag
...
;;    flg_NO2(n:n+(npix-lp)-1) = trp_flg(lp/2:npix-(lp/2+1),m)
    xtrack_flag_NO2(n:n+(npix-lp)-1) =xtrack_flag(lp/2:npix-(lp/2+1),m)
...
;ind_ok = where((time_NO2 GT 0) AND (flg_NO2 EQ 0) AND (FINITE(value_NO2) EQ 1) AND (sza_NO2 LT 70.) AND $
 ind_ok = where((time_NO2 GT 0) AND  (value_NO2 ge 0) AND (sza_NO2 LT 70.) AND (sza_NO2 GE 0.)$
	       ...
	       (cfr_NO2/1000. GE 0. AND cfr_NO2/1000. LE 0.3) AND           $
               ...
               ((xtrack_flag_NO2 EQ 0) or (xtrack_flag_NO2 EQ 4)) AND           $
		 ; (tramf_NO2 GT 0.5) 
